...
```tsx
import { CellSelectionPlugin } from '@basestacks/datagrid';
import { useDataGrid, usePlugin } from '@basestacks/datagrid/react';
...
> **Note:** On macOS, use the **Command (⌘)** key. On Windows/Linux, use the **Ctrl** or **Alt** key, depending on the platform.
...
- **Shift + Arrow Keys**: Expands the current selection by one cell in the arrow’s direction.
- **Ctrl + Shift + Arrow Keys**: Expands the selection to the edge of the data region in the arrow’s direction.
